...
dataset_files_all = []
dataset_files_ = [:]
dataset_files_only = [] // Full dataset, AIBST only for now
dataset_files_chr = []
dataset_files_pop = []
...
dataset_samples = [] //pop, dataset, dataset_sample_file
dataset_files = [] // dataset, chrm, vcf_file_chrm
params.dataset_files.each { dataset, dataset_vcf_file, dataset_sample_file, dataset_population_list ->
    if(!file(dataset_sample_file).exists()) exit 1, "File ${dataset}:${dataset_sample_file} not found. Please check your config file."
    dataset_population_list.split(',').each { pop ->
        dataset_samples << [pop, dataset, file(dataset_sample_file)]
    }
    if(dataset in params.dataset_files_phasing){
        CHRMS.each { chrm ->
            dataset_chrm = file(sprintf(dataset_vcf_file, chrm))
            if(!file(dataset_chrm).exists()) exit 1, "File ${dataset}:${dataset_chrm} not found. Please check your config file."
            dataset_files << [dataset, chrm, dataset_chrm]
        }
    }
}

"""
Step 1: Subset sample file
"""
process subset_sample_file {
    tag "subset_sample_file_${dataset}_${pop}"
    label "small"
    input:
    set pop, dataset, file(sample_file) from dataset_samples

    output:
    set pop, dataset, file(pop_file) into subset_sample_file

    script:
    pop_file = "${pop}_${dataset}_${params.subset_size}_updated.sample"
    """
    grep ${pop} ${sample_file} | awk '{ \$2=\$2"\\t${dataset}"; print \$0}' | sort -R | tail -n ${params.subset_size} > ${pop_file}
    """
}

def func_all_datasets = { pop, dataset, pop_file ->
    all_datasets = []
    params.dataset_files.each { dataset_, dataset_vcf_file, dataset_sample_file, dataset_population_list ->
        CHRMS.each { chrm ->
            dataset_chrm = file(sprintf(dataset_vcf_file, chrm))
            if(!file(dataset_chrm).exists()) exit 1, "File ${dataset_}:${dataset_chrm} not found. Please check your config file."
            if(dataset == dataset_){
                all_datasets << [dataset, file(dataset_chrm), pop, file(pop_file), chrm]
            }
        }
    }
    return all_datasets
...
dataset_files_pop = subset_sample_file
        .flatMap{ it -> func_all_datasets(it) }
...
"""
Step 2: Split dataset per population
"""
process split_dataset_pop {
    tag "split_dataset_${dataset}_${pop}_${chrm}"
    label "medmem"
    input:
    set dataset, file(dataset_chrm_vcf), pop, file(pop_sample), chrm from dataset_files_pop
...
"""
Step 3: Combine chromosomesto one by population
"""
...
process concat_dataset_pop {
    ...
    output:
    set pop, file(vcf_out), sample, dataset into concat_dataset_pop,concat_dataset_pop_groups,concat_dataset_pop_groups_1
...
params.dataset_groups.each{ dataset_group ->
    ...
    if(datas.size() > 1){
        dataset_pop_groups << datas
    }
}

"""
Step 4: Merge populations into group
"""
process merge_pop_groups {
    tag "merge_pop_groups_${group}"
    label "medium"
    input:
    set group, pop_vcfs, pop_samples from dataset_pop_groups

    output:
    set group, file(vcf_out), file(sample_out) into merge_pop_groups

    script:
    vcf_out = "${group}.merged.vcf.gz"
    sample_out = "${group}.sample"
    """
    bcftools merge ${pop_vcfs.join(' ')} -Oz -o ${vcf_out}
...
"""
Step 5: Recalculate AC, AF, MAF and Sort VCF
"""
process sort_pop_groups {
    tag "sort_pop_groups_${group}"
    label "extrabig"
    input:
    set group, file(group_vcf), file(group_sample) from merge_pop_groups

    output:
    set group, file(vcf_out), file(group_sample) into sort_pop_groups

    script:
    vcf_out = "${group}.sorted.vcf.gz"
    """
    ## Recalculate AC, AN, AF
    bcftools +fill-tags ${group_vcf} | \
    bcftools sort --temp-dir ${params.tmpdir} -Oz -o ${vcf_out}
    """
}


"""
Step 6: QC, Set IDs to POS_REF_ALT
"""
process qc_pop_groups {
    tag "qc_pop_groups_${group}"
    label "extrabig"
    input:
    set group, file(group_vcf), file(group_sample) from sort_pop_groups

    output:
    set group, file("${vcf_out_qc}.vcf.gz"), file(group_sample) into qc_pop_groups_plink,qc_pop_groups_adme

    script:
    vcf_out_id = "${group}_qc.vcf.gz"
    vcf_out_qc = "${group}_qc_miss10_maf001"
    """
    bcftools annotate \
        -x INFO,^FORMAT \
        --set-id '%CHROM\\_%POS\\_%REF\\_%ALT' ${group_vcf} -Oz -o ${vcf_out_id}
    vcftools --gzvcf ${vcf_out_id} \
        --max-missing 0.8 \
        --maf 0.01 \
        --out ${vcf_out_qc} \
        --recode --recode-INFO-all \
        --stdout | bgzip > ${vcf_out_qc}.vcf.gz
    """
}


"""
Step 6.1: QC, Set IDs to POS_REF_ALT for pgx only
"""
qc_pop_groups_pgx_ch = qc_pop_groups_adme.combine([file(params.gene_regions_slop)])
process qc_groups_subset_pgx {
    tag "qc_groups_subset_pgx_${group}"
    label "medmem"
    // publishDir "${params.work_dir}/data/${dataset}/PGX_ONLY/ANN/CHRS", mode:'symlink'

    input:
    set group, file(group_vcf), file(group_sample), file(gene_regions_slop) from qc_pop_groups_pgx_ch

    output:
    set group_pgx, file(vcf_out), file(group_sample) into qc_groups_subset_pgx

    script:
    vcf_out = "${file(group_vcf.baseName).baseName}_pgx.vcf.gz"
    group_pgx = "${group}_pgx"
    """
    tabix ${group_vcf}
    bcftools view \
        ${group_vcf} \
...
"""
Step 7: Convert from VCF to plink for PCA analysis
"""
process group_vcf_to_plink {
    tag "group_vcf_to_plink_${group}"
    label "bigmem"
    input:
        set group, file(group_vcf), file(group_sample) from qc_pop_groups_plink.mix(qc_groups_subset_pgx)
    output:
...
"""
Step 8: Run Smartpca PCA analysis for group
"""
...
"""
Step 9: Add group to evec file from smartpca
"""

process update_evec {
    tag "update_evec_${group}"
    label "medmem"
    publishDir "${params.work_dir}/data/${group}/ALL/VCF", mode: 'symlink'
    input:
...
"""
Step 10: Plot PCA analysis for group
"""
...
'''
Step 11: Prepare all data to be used
'''
...
"""
Step 12: Filter Biallelic sites only in VCFs AIBST
"""
process biall_dataset {
    ...
    output:
    set val(dataset), val(chrm), file(vcf_out) into biall_dataset,biall_dataset_1,biall_dataset_2
...
"""
Step 13: Phase VCFs AIBST using eagle
"""
process phase_dataset {
    ...
    input:
    set val(dataset), val(chrm), file(vcf_file) from biall_dataset

    output:
    set val(dataset), val(chrm), file("${vcf_out}.vcf.gz") into phase_dataset,phase_dataset_1
...
process annotate_dataset_snpeff{
    ...
    input:
    set val(dataset), val(chrm), file(vcf_file) from phase_dataset

    output:
    set val(dataset), val(chrm), file("${vcf_out}.gz") into annotate_dataset_snpeff
...
process annotate_dataset_dbsnp{
    ...
    input:
    set val(dataset), val(chrm), file(vcf_file) from annotate_dataset_snpeff

    output:
    set val(dataset), val(chrm), file("${vcf_out}.gz") into annotate_dataset_dbsnp,annotate_dataset_dbsnp_6
...
process annotate_dataset_gwascat{
    ...
    input:
    set val(dataset), val(chrm), file(vcf_file) from annotate_dataset_dbsnp

    output:
    set val(dataset), val(chrm), file("${vcf_out}.gz") into annotate_dataset_gwascat
...
process annotate_dataset_clinvar {
    ...
    input:
    set val(dataset), val(chrm), file(vcf_file) from annotate_dataset_gwascat

    output:
    set val(dataset), val(chrm), file("${vcf_out}.gz")into annotate_dataset_clinvar
...
process annotate_cosmic_baylor {
    ...
    input:
    set val(dataset), val(chrm), file(vcf_file) from annotate_dataset_clinvar

    output:
    set val(dataset), val(chrm), file("${vcf_out}.gz") into annotate_dataset_cosmic
...
// mafs_annot_dataset.into { mafs_annot_dataset; mafs_annot_dataset_1; mafs_annot_dataset_2 }
// annotate_dataset_cosmic.into { annotate_dataset_cosmic; annotate_dataset_cosmic_1; annotate_dataset_cosmic_3}
...
// annotate_dataset_cosmic_list.each{ dataset, chrm, vcf_file ->
...
// }
// annotate_dataset_cosmic_2_cha = Channel.from(annotate_dataset_cosmic_2.values()).view()
annotate_dataset_cosmic_mafs = annotate_dataset_cosmic
                                .join(mafs_annot_dataset.groupTuple(by:1), by:1).map{it->[it[0],it[1],it[2],it[3].sort().join('-'), it[4].sort().join(';')]}
...
process annotate_mafs_dataset {
    ...
    input:
    set chrm, val(dataset), file(vcf_file), val(mafs), val(mafs_files) from annotate_dataset_cosmic_mafs

    output:
    set val(dataset), val(chrm), file(outVCF) into annotate_mafs_dataset

    script:
    outVCF = "${file(vcf_file.baseName).baseName}_mafs-${mafs}.vcf"
    inVCF = vcf_file
    inTSV = mafs_files
    outTSV = ''
    template "annotateVCFwithTSV.py"
...
split_POP_samples.toSortedList().val.each { pop, pop_sample, dataset ->
...
annotate_mafs_dataset_gz_1.join(merge_pop_sample, by:0).view()
